...
const CartProductsMobile = () => {
  ...
  return (
    <Box bg='ghostwhite'  width='100%' padding={isLessWidthThan565?'20px 20px 20px 5%':'20px 40px 20px 5%'}>
      <Text fontSize='xl'>Your Cart</Text>
      {userStore.cart.products.map((product,index)=>
      <>
      {index >0 &&<hr style={{border:'0.5px solid var(--chakra-colors-gray-200)',margin:'10px 0'}}/>}
       <Flex key={product.gameName} width='100%' height='100px'  justifyContent='flex-start' padding='10px 0' mt={index ==0?4:2} alignItems='flex-start'> 
        ...
        <QuantitySelectorMobile limit={10} changeQuantity={(newQuantity)=>handleUpdateQuantity(product.gameName,product.platform,newQuantity)}/>
        </Stack>
        ...
        <Flex justifyContent='center' alignItems='center' cursor='pointer'
         onClick={()=>handleRemoveFromCart(product.gameName,product.platform)}>
          <Icon as={ImBin} boxSize={5} mr={1} mt={1} color='gray.700'/>
