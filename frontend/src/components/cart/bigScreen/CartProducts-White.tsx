  ...
  return (
     <Box bg='ghostwhite' width='60%' padding='50px 20px 100px 5%'> 
      ...
      <>
        ...
        <Flex key={product.gameName} width='100%' height='150px' boxShadow='2px 2px 8px rgba(0, 0, 0, 0.2)' justifyContent='flex-start' padding='10px' mt={index ==0?5:8} alignItems='flex-start'>  
        ...
        {/* <QuantitySelector initialQuanity={product.quantity} */}
        ...
        <Flex justifyContent='center' alignItems='center' cursor='pointer'
         onClick={()=>handleRemoveFromCart(product.gameName,product.platform)}>
          <Icon as={ImBin} boxSize={6} mt={2} color='gray.700'/>
          <Text mt={2}>delete</Text>
        </Flex>
        <Text fontSize='lg' >{(product.price*product.quantity).toFixed(2)}&euro;</Text>
        </Stack>
        </Flex>
        </>
      )}
...
export default CartProductsWhite